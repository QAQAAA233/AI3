...
def load_project():
    ...
    try:
        ...
        metadata_lookup = {item['filename']: item for item in project_info.get('files', []) or []}
        attached_files = []
        for file in project_files:
            meta = metadata_lookup.get(file['name'])
            if meta:
                attached_files.append({
                    'name': file['name'],
                    'type': file['type'],
                    'description': meta.get('description', '現有檔案'),
                })
            else:
...
def run_process():
    ...
    try:
        ...
        if not all([folder_path, prompt]):
            return jsonify({'success': False, 'error': '缺少必要參數(資料夾路徑或 AI 指令)', 'ai_response': ''}), 400

        config = AIConfig(**{key: value for key, value in config_data.items() if key != 'response_mode'})
